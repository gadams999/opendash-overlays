...
description: "All Discord Chat Overlay settings fields explained with defaults and valid ranges"
---

# Settings Reference

All settings are stored in `%APPDATA%\DiscordChatOverlay\settings.json` and loaded automatically on startup. Changes made in the settings window take effect immediately (live preview) and are written to disk on **Save**.
...
```
%APPDATA%\DiscordChatOverlay\settings.json
```
...
```
%APPDATA%\DiscordChatOverlay\aliases.json
```
...
```
%APPDATA%\DiscordChatOverlay\logs.txt
```
