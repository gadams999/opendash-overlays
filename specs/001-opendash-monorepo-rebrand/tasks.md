...
- [ ] T001 [P] Delete scaffold file `src/OverlayCore/Placeholder.cs` (no longer needed now that real OverlayCore services exist)
- [ ] T002 [P] Delete scaffold file `tests/OverlayCore.Tests/PlaceholderTests.cs` (replaced by real property tests)
- [ ] T003 Update `scripts/Validate-PropertyTests.ps1` — add optional `-TestProjectPath` parameter (default: `tests/WheelOverlay.Tests`) so the script can validate any test project; update CI workflow calls to pass `-TestProjectPath` explicitly for both test projects
