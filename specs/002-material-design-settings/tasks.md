...
- [ ] T021 [P] Performance spot-check: launch WheelOverlay with the settings window open and leave idle for 30 seconds; confirm working-set RAM stays below 50 MB via `(Get-Process WheelOverlay).WorkingSet64 / 1MB`; confirm CPU stays below 2% by sampling Task Manager's CPU column visually for 10 seconds, or via two-point delta: `$a=(Get-Process WheelOverlay).CPU; Start-Sleep 10; $b=(Get-Process WheelOverlay).CPU; "{0:F2}% avg" -f (($b-$a)/10*100/[Environment]::ProcessorCount)` — verify MDIX library load has not caused measurable regression against NFR-001 (spec.md §Assumptions ¶6, plan.md §Technical Context)
