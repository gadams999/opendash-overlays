# Build and Publish Script for DiscordChatOverlay
# Full release build: clean, build, test (Release), build MSI, sign if cert available
$ErrorActionPreference = "Stop"

# Resolve repo root (two levels up from scripts/discord-chat-overlay/)
$repoRoot = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
Push-Location $repoRoot

$projectPath = ".\src\DiscordChatOverlay\DiscordChatOverlay.csproj"
$publishDir = ".\Publish"

Write-Host "=== DiscordChatOverlay Release Build ===" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "Application published to: $publishDir\DiscordChatOverlay.exe" -ForegroundColor Green
Write-Host ""
...
Write-Host "MSI built: .\Package\DiscordChatOverlay-v$version.msi" -ForegroundColor Green
Write-Host ""
...
if ($signtool) {
    ...
    Write-Host "  To sign: signtool sign /fd SHA256 /tr http://timestamp.digicert.com /td SHA256 Package\DiscordChatOverlay-v$version.msi" -ForegroundColor Gray
} else {
...
Write-Host "  MSI:         .\Package\DiscordChatOverlay-v$version.msi" -ForegroundColor White
Write-Host "  Executable:  .\Publish\DiscordChatOverlay.exe" -ForegroundColor White
Write-Host ""
